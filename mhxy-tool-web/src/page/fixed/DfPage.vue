<template>
	<div>
		<s-title>地府固定伤害计算</s-title>
		<s-input v-model="jineng" :val="jineng" label="阎罗令技能:"></s-input>
		...
		<s-result v-model="result" label="白天伤害结果:">{{result}}</s-result>
		<s-result v-model="result2" label="晚上伤害结果:">{{result2}}</s-result>
	</div>
</template>
    ...
    export default {
        ...
        methods: {
            getResult: function () {
                ...
                fixParam(param, 0);
				axios.get(baseUrl + '/fixedDamage/df', {
				    params: param
